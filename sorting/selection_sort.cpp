...
void selection_sort(vector<int> &lista_nums){
    ...
    for(int i= 0; i< lista_nums.size(); i++){
        ...
        //swap
        int num = lista_nums[i];
        lista_nums[i] = lista_nums[current_min];
        lista_nums[current_min] = num;
    }
}
//----------------------------------------------------------------------------------------//
...
int main(){
    ...
    vector<int> listado_ordenado = merge_sort(listado);

    for (int e: listado_ordenado){
        cout << e << ' ';
    }
    cout << endl;
